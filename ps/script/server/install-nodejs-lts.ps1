...
# Installs Node.js LTS with npm and essential global packages
...
function Test-NodeJSInstalled {
        ...
        if ($nodeVersion) {
            ...
            if ($nodeVersion -match "v(\d+)\.") {
                ...
                if ($majorVersion % 2 -eq 0 -and $majorVersion -ge 18) {
                    Write-Host "     ✓ LTS version" -ForegroundColor Green
                } else {
                    Write-Host "     ⚠ Not LTS version" -ForegroundColor Yellow
                }
            }
        }
    } catch {
    ...
    return ($nodeInstalled -and $npmInstalled)
}
