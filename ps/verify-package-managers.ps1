...
if ($failedManagers) {
    ...
    foreach ($failed in $failedManagers) {
        switch ($failed.Name) {
            "Chocolatey" {
                Write-Info "To install Chocolatey: Set-ExecutionPolicy Bypass -Scope Process -Force; iex ((New-Object System.Net.WebClient).DownloadString('https://community.chocolatey.org/install.ps1'))"
            }
            "Scoop" {
                Write-Info "To install Scoop: Set-ExecutionPolicy RemoteSigned -Scope CurrentUser; irm get.scoop.sh | iex"
            }
            "WinGet" {
                Write-Info "WinGet should be pre-installed. Try restarting PowerShell or install from Microsoft Store."
            }
        }
    }
}